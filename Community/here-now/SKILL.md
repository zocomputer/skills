---
name: here-now
description: here.now gives Zo a fast publish-to-web path for files, folders, reports, dashboards, and static sites.
metadata:
  author: here.now
  category: Community
  display-name: here.now
  emoji: 🌐
---

# here.now

here.now gives Zo a fast publish-to-web path for files, folders, reports, dashboards, and static sites.
...
```bash
./scripts/publish.sh {file-or-dir} --client zo
```
...
```bash
./scripts/publish.sh {file-or-dir} --slug {slug} --client zo
```
...
Pass `--client zo` so here.now can track reliability by agent:

```bash
./scripts/publish.sh {file-or-dir} --client zo
```

This sends `X-HereNow-Client: zo/publish-sh` on publish API calls.
...
| `--client {name}`      | Agent name for attribution (e.g. `zo`)        |
