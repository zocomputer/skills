...
Usage:
  ...
  mengram_memory.py recall "query"      - Gated semantic search (skips KG for irrelevant queries)
  mengram_memory.py recall-raw "query"  - Raw semantic search (always queries KG)
...
class MengramMemory:
    ...
    def recall(self, query, top_k=5):
        """Gated recall - uses gate model to skip irrelevant queries."""
        return self._request("POST", "/api/recall/gated", {"query": query, "top_k": top_k})

    def recall_raw(self, query, top_k=5):
        """Raw recall - always queries knowledge graph."""
        return self._request("POST", "/api/recall", {"query": query, "top_k": top_k})

    def search(self, query, top_k=5):
        """Gated search - uses gate model to skip irrelevant queries."""
        return self._request("POST", "/api/search/gated", {"query": query, "top_k": top_k})

    def search_raw(self, query, top_k=5):
        """Raw search - always queries knowledge graph."""
...
def main():
    ...
    if command == "initialize":
        ...
        if decision.get("needs_memory") is False:
            print(f"[Gate: SKIP - {decision.get('reason', 'no memory needed')}]")
        elif context:
            print(context)
        else:
            print("No relevant memories found.")

    elif command == "recall-raw":
        if len(sys.argv) < 3:
            print("Usage: mengram_memory.py recall-raw \"query\"")
            sys.exit(1)
        query = sys.argv[2]
        result = client.recall_raw(query)
        context = result.get("context", "")
        if context:
            print(context)
        else:
