...
export default async (c: Context): Promise<Response> => {
  ...
  if (!input) {
    return jsonError({ error: "missing_input" }, 400, cors);
  }
  if (input.length > MAX_INPUT_LEN) {
    return jsonError({ error: "input_too_long", max: MAX_INPUT_LEN }, 400, cors);
  }

  const upstreamPayload: Record<string, unknown> = { input };
  if (typeof body.model_name === "string" && body.model_name) {
    upstreamPayload.model_name = body.model_name;
  }
  if (typeof body.persona_id === "string" && body.persona_id) {
    upstreamPayload.persona_id = body.persona_id;
  }
  ...
  try {
    ...
    console.error("[alaric-ask] fetch failed", err);
  ...
  if (upstream.status === 401 || upstream.status === 403) {
    return jsonError(
      { error: "invalid_zo_token", detail: upstreamText.slice(0, 300) },
      401,
      cors,
    );
  }
  ...
  if (!upstream.ok) {
    console.error("[alaric-ask] upstream error", upstream.status, upstreamText.slice(0, 300));
    return jsonError(
      { error: "upstream_error", status: upstream.status, detail: upstreamText.slice(0, 300) },
      502,
      cors,
    );
  }
